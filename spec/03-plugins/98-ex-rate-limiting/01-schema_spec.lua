local schemas = require "kong.dao.schemas_validation"
local plugin_schema = require "kong.plugins.ex-rate-limiting.schema"
local validate_entity = schemas.validate_entity

describe("Plugin: ex-rate-limiting (schema)", function()
  it("proper config validates", function()
    local config = {second = 10}
    local ok, _, err = validate_entity(config, plugin_schema)
    assert.True(ok)
    assert.is_nil(err)
  end)
  it("proper config validates (bis)", function()
    local config = {second = 10, hour = 20}
    local ok, _, err = validate_entity(config, plugin_schema)
    assert.True(ok)
    assert.is_nil(err)
  end)
  it("set limit_by to header",function()
    ocal config = {limit_by="header",limit_by_value="X-REAL-IP"}
      local ok, _, err = validate_entity(config, plugin_schema)
      assert.False(ok)
      assert.is_nil(err)
  end)

  describe("errors", function()
    it("limits: smaller unit is less than bigger uni", function()
      local config = {second = 20, hour = 10}
      local ok, _, err = validate_entity(config, plugin_schema)
      assert.False(ok)
      assert.equal("The limit for hour cannot be lower than the limit for second", err.message)
    end)
    it("invalid limit", function()
      local config = {}
      local ok, _, err = validate_entity(config, plugin_schema)
      assert.False(ok)
      assert.equal("You need to set at least one limit: second, minute, hour, day, month, year", err.message)
    end)
    it("invalid limit_by",function()
      local config = {limit_by="header"}
      local ok, _, err = validate_entity(config, plugin_schema)
      assert.False(ok)
      assert.equal("You need to set limit_by_value when set limit_by to 'header':", err.message)
    end)
  end)
end)
