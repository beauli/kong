...
for i, policy in ipairs({"local", "cluster", "redis"}) do
  describe("#ci Plugin: ex-rate-limiting (access) with policy: "..policy, function()
    setup(function()
      helpers.kill_all()
      flush_redis()
    ...
    describe("With authentication", function()
      ...
      describe("Plugin customized for specific consumer", function()
        ...
        it("blocks if the only ex-rate-limiting plugin existing is per consumer and not per API", function()
          for i = 1, 6 do
            local res = assert(client:send {
              method = "GET",
              path = "/status/200/?apikey=apikey122",
